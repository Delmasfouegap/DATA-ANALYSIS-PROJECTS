...
create view totalOrder as select orderNumber, sum(quantityOrdered*priceEach) as tot
from orderdetails group by orderNumber;
select *from totalOrder;
select tot from totalOrder where orderNumber=10102;
#drop view totalOrdini # per cancellare una vista
show  create view  totalOrder;

--
CREATE USER  pippo@localhost
IDENTIFIED BY  'pluto';
SHOW VARIABLES LIKE 'validate_password%'; #non ha funzionato...


CREATE PROCEDURE bonjour();
BEGIN
  SELECT "Hello Styll Gabana", 4*4
  select "gros genie"
END$$;
call bonjouuuur();
...
show procedure status;
  call my_procedure();
#drop procedure bonjour # Per cancelare une procedura

create procedure  Myproc( tipo varchar(50))
begin
select  *from employees where jobTitle = tipo;
end ;
  call Myproc("sales rep");
  show  procedure status;
